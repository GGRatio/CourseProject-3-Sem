...
int BinaryToDecimal(std::string input) {
	try {
		...
		size_t startIdx = 2;
		if (isNegative) startIdx = 3;
		int value = std::stoi(input.substr(startIdx), nullptr, 2);
...
namespace Lexer
{
	Graph graphs[N_GRAPHS] =
	{
		...
		{ LEX_ID_TYPE, FST::FST(GRAPH_BYTE) },
		...
		{ LEX_LITERAL_BIN, FST::FST(GRAPH_BIN_LITERAL) }
	};
		...
		bool& lex_ok)
	{
		...
		else
		{
			...
			if (isFunc)
			{
				...
				case IT::STDFNC::F_ABS:
				{
					itentry->idtype = IT::IDTYPE::S;
					itentry->iddatatype = ABS_TYPE;
					itentry->value.params.count = ABS_PARAMS_CNT;
					itentry->value.params.types = new IT::IDDATATYPE[ABS_PARAMS_CNT];
					for (int k = 0; k < ABS_PARAMS_CNT; k++)
						itentry->value.params.types[k] = IT::ABS_PARAMS[k];
					break;
				}
			...
			for (int j = 0; j < N_GRAPHS; j++)
			{
				...
				if (FST::execute(fst))
				{
					...
					case LEX_SEPARATORS:
					{
						...
						case LEX_LEFTBRACE:
						{
							if (i > 0 && *in.words[i - 1].word == LEX_ISFALSE || *in.words[i - 1].word == LEX_ISTRUE || *in.words[i - 1].word == LEX_DO)
								break;
							char* functionname = new char[MAXSIZE_ID];
							char* scopename = getScopeName(tables.idtable, in.words[i - 1].word);
					...
					{
						int value = BinaryToDecimal(curword);
					...
					case LEX_LITERAL:
					{
						...
						else
						{
							...
							// Ошибка 305 должна вызываться только при повторном объявлении,
							// т.е. когда перед идентификатором есть LEX_VAR или LEX_FUNCTION
