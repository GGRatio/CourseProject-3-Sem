...
namespace MFST
{
	...
	bool Mfst::start(const Log::LOG& log)
	{
		...
		case LENTA_END:         MFST_TRACE4(log, "------>LENTA_END")
			* log.stream << "-------------------------------------------------------------------------------------" << std::endl;
			sprintf_s(buf, MFST_DIAGN_MAXSIZE, "%d: Input size %d, syntax analysis completed successfully ", 0, lenta_size);
			*log.stream << std::setw(4) << std::left << 0 << ": Input size " << lenta_size << ", syntax analysis completed successfully " << std::endl;
			rc = true;
			break;
			...
			* log.stream << "-------------------------------------------------------------------------------------" << std::endl;
			{
				...
				*log.stream << diag0 << std::endl;
				...
				char buf1[MFST_DIAGN_MAXSIZE];
				char* diag1 = getDiagnosis(1, buf1);
				if (strlen(diag1) > 0)
					*log.stream << diag1 << std::endl;
				
				char buf2[MFST_DIAGN_MAXSIZE];
				char* diag2 = getDiagnosis(2, buf2);
				if (strlen(diag2) > 0)
